...
TEST(Pattern, Basic_3) {
  ...
  ASSERT_TRUE((px - floormod(py, px * PConst<Expr>(2))).Match(x - floormod(2, x * 2)));
}
...
TEST(Pattern, Select) {
  ...
  }
  {
    ...
    ASSERT_EQ(pz.Eval().Cast<IntImmObj>()->value, 1);
  }
...
TEST(Pattern, Ramp) {
  ...
  PVar<Expr> px;
  PVar<Expr> lanes;
  ASSERT_TRUE(ramp(px, PConst<Expr>(1), lanes).Match(Ramp(x, 1, 10)));
  ASSERT_TRUE(lanes.Eval().Cast<IntImmObj>()->value == 10);
  ASSERT_TRUE(!ramp(px, PConst<Expr>(1), lanes).Match(Ramp(x, 2, 10)));
}
