...
template <> class PEqualChecker<StringImm> {
public:
  bool operator()(const StringImm &lhs, const StringImm &rhs) const { return lhs->value == rhs->value; }
};
...
public:
  ...
  bool Match_(const NodeRefType &value) const {
    if (const auto *ptr = value.template TryCast<typename T::TObj>()) {
      return Match_(T(ptr));
    } else {
      return false;
    }
...
public:
  ...
  bool Match_(const ObjectRef &node) const {
    ...
    if (const NodeType *ptr = node.TryCast<NodeType>()) {
      if (!a_.Match_(ptr->a))
        return false;
      if (!b_.Match_(ptr->b))
        return false;
...
public:
  ...
  bool Match_(const ObjectRef &node) const {
    if (const IntImmObj *ptr = node.TryCast<IntImmObj>()) {
  ...
  Expr Eval() const { return IntImm(value_); }
...
public:
  ...
  bool Match_(const ObjectRef &node) const {
    if (const NotObj *ptr = node.TryCast<NotObj>()) {
...
public:
  ...
  bool Match_(const ObjectRef &node) const {
    if (const SelectObj *ptr = node.TryCast<SelectObj>()) {
...
public:
  ...
  bool Match_(const ObjectRef &node) const {
    if (const CastDTypeObj *ptr = node.TryCast<CastDTypeObj>()) {
...
public:
  ...
  bool Match_(const ObjectRef &node) const {
    if (const RampObj *ptr = node->TryCast<RampObj>()) {
...
inline PRampExpr<TBase, PConstWithTypeLike<TBase>, PConstWithTypeLike<TBase>> ramp(const Pattern<TBase> &base,
...
public:
  ...
  bool Match_(const ObjectRef &node) const {
    if (const BroadcastObj *ptr = node->TryCast<BroadcastObj>()) {
      if (!value_.Match_(ptr->value))
        return false;
      if (!lanes_.Match_(ptr->lanes))
        return false;
...
public:
  ...
  bool Match_(const ObjectRef &node) const {
    if (const CallObj *ptr = node->TryCast<CallObj>()) {
      if (ptr->args.size() != sizeof...(TArgs))
        return false;
      if (ptr->op.TryCast<OpObj>() != Op::GetOp().get())
        return false;
...
#define MLC_SYM_PM_UNARY_INTRIN(FuncName, OpName, IntrinOp)                                                            \
  struct OpName {                                                                                                      \
    static Expr Eval(List<Expr> args) { return Call(OpName::GetOp(), args); }                                          \
